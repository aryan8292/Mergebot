...
async def loginHandler(c: Client, m: Message):
    ...
    if user.allowed:
        ...
        if passwd == Config.PASSWORD:
            ...
            await m.reply_text(
                text=f"**Login failed ❌,**\n  🛡️ Send Correct password to use me\n\nContact: 🈲 @{Config.OWNER_USERNAME}",
                quote=True,
            )
    user.set()
    del user
    ...
    if m.from_user.id != int(Config.OWNER):
        if user.allowed is False:
            res = await m.reply_text(
                text=f"Hi **{m.from_user.first_name}**\n\n 🛡️ Send Command /login then send password to use me\n\n**Contact: 🈲 @{Config.OWNER_USERNAME}** ",
                quote=True,
            )
            return
    else:
    ...
    if user.merge_mode == 4:
        ...
        if rmess.video or rmess.document:
            ...
            if file_name is None:
                await m.reply("File name not found; goto @mdiskbot200")
                return
...
🔰 ᴏᴡɴᴇʀ ᴄᴀɴ ʙʀᴏᴀᴅᴄᴀsᴛ ᴍᴇssᴀɢᴇ ᴛᴏ ᴀʟʟ ᴜsᴇʀs
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("🍿Movies lover🍿", url="https://t.me/allmoviesherejoinhindi")],
                [
                    InlineKeyboardButton(
                        "🛸Updates Channel🛸", url="https://t.me/nickallbots"
                    ),
                    InlineKeyboardButton(
    ...
    try:
        with userBot:
            userBot.send_message(
                ...
                text="Bot booted with Premium Account,\n\n  Thanks for using <a href='https://t.me/Merge_video_bot'>this bot</a>",
